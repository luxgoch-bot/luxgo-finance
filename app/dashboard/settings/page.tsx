...
import { Settings, Building2, CalendarDays, Bell, Download, Plus, Check, X } from 'lucide-react'
                ...
                <CardContent className="space-y-3">
                  {addingYear && (
                    <div className="flex items-center gap-2 p-3 rounded-lg bg-gray-800 border border-gray-700">
                      <Input
                        ...
                        onKeyDown={e => { if (e.key === 'Enter') addTaxYear() }}
                        autoFocus
                      />
                      <Button size="sm" onClick={addTaxYear} className="bg-amber-500 hover:bg-amber-400 text-black">
                        <Check className="h-4 w-4" />
                      </Button>
                      <Button size="sm" variant="outline" onClick={() => setAddingYear(false)}
                  ...
                  {taxYears.length === 0 && !addingYear ? (
                    <p className="text-gray-500 text-sm py-4 text-center">
                      No tax years yet — add one to get started
                    </p>
                  ) : (
                    taxYears.map(ty => {
                ...
                <CardContent className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <Select value={exportYear} onValueChange={setExportYear}>
                      <SelectTrigger className="w-48 bg-gray-800 border-gray-700 text-white focus:border-amber-500">
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <div className="rounded-lg bg-gray-800/50 border border-gray-800 px-4 py-3 text-xs text-gray-500">
                    💡 CSV files use UTF-8 with BOM for correct Excel display of Swiss characters (ä, ö, ü).
                    Open in Excel → Data → From Text/CSV for best results.
                  </div>
                </CardContent>
              </Card>
            </TabsContent>
